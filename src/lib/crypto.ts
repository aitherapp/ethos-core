...
// Factory API (MlKem1024 class is deprecated in crystals-kyber-js v2.7+)
...
export async function generateIdentity(): Promise<QuantumIdentity> {
  ...
  // 2. Post-Quantum Kyber-1024 (init async; keygen/encap/decap are sync)
...
export async function deriveHybridSecret(
  ...
  if (isInitiator) {
    ssPqc = kem.decap(b64decode(peerPqcPKOrCT), identity.pqcPrivateKey);
  } else {
    const [ct, ss] = kem.encap(b64decode(peerPqcPKOrCT));
    ssPqc = ss;
...
function bufferSource(bytes: Uint8Array): ArrayBufferBytes {
...
function b64decode(str: string): ArrayBufferBytes {
...
export async function doesPublicKeyMatchPeerId(peerId: string, publicKey: string): Promise<boolean> {
  if (!peerId || !publicKey) return false;
  const expectedPeerId = await hashId(publicKey);
  if (peerId.length === 16) return expectedPeerId.startsWith(peerId);
  return expectedPeerId === peerId;
}
...
async function kdfRatchet(rootKeyBytes: Uint8Array, dhOutput: Uint8Array): Promise<{ rootKey: Uint8Array; chainKey: CryptoKey }> {
  ...
  const derive = async (key: Uint8Array, info: Uint8Array): Promise<CryptoKey> => {
    return wcs.deriveKey(
      { name: 'HKDF', salt: new Uint8Array(0), info: bufferSource(info), hash: 'SHA-256' },
      await wcs.importKey('raw', bufferSource(key), 'HKDF', false, ['deriveKey']),
      { name: 'AES-GCM', length: 256 },
...
export async function initializeRatchet(secretBytes: string, isInitiator: boolean): Promise<RatchetState> {
  ...
  const rootKey = await wcs.importKey('raw', bufferSource(keyInput), 'AES-GCM', false, ['encrypt', 'decrypt']);
...
export async function deriveMessageKey(chainKey: CryptoKey): Promise<{ messageKey: CryptoKey; nextChainKey: CryptoKey }> {
  ...
  const hkdfKey = await wcs.importKey('raw', chainKeyBytes, 'HKDF', false, ['deriveBits']);
  const derived = await wcs.deriveBits(
    {
      name: 'HKDF',
      salt: new Uint8Array(0),
      info: bufferSource(MESSAGE_KEY_INFO),
      hash: 'SHA-256',
    },
    hkdfKey,
    512
  ...
  // Message keys are not extractable; chain keys must be extractable for the next ratchet step.
  const msgKeyData = derivedBytes.slice(0, 32);
  const messageKey = await wcs.importKey('raw', msgKeyData, 'AES-GCM', false, ['encrypt', 'decrypt']);
  
  const nextChainData = derivedBytes.slice(32, 64);
  const nextChainKey = await wcs.importKey('raw', nextChainData, 'AES-GCM', true, ['encrypt', 'decrypt']);
